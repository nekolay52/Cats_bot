...
async def hello_world(callback : CallbackQuery, state : FSMContext):
    ...
    await state.update_data(cat_message_id=media.message_id)
...
async def hello_world(callback : CallbackQuery, state : FSMContext):
    ...
    temp_data = await state.get_data()
    await bot.edit_message_media(media=media, chat_id=callback.message.chat.id, message_id=temp_data['cat_message_id'], reply_markup=button_watch_cats)
...
async def hello_world(callback : CallbackQuery, state : FSMContext):
    media = types.InputMediaPhoto(media=types.FSInputFile(path=f"users_pictures/{callback.from_user.id}/temp.png"), caption="Котик")
    temp_data = await state.get_data()
    await bot.edit_message_media(media=media, chat_id=callback.message.chat.id, message_id=temp_data['cat_message_id'], reply_markup=list_menu(callback.from_user.id, "append_in_cat"))
...
@router.callback_query(F.data == 'Назад_1')
async def hello_world(callback : CallbackQuery, state : FSMContext):
    get_cat(f"users_pictures/{callback.from_user.id}/temp.png")
    temp_data = await state.get_data()
    await bot.edit_message_text(text="О привет! Выбери пожалуйста что ты хочешь делать", chat_id=callback.message.chat.id, message_id=temp_data['message_main_id'], reply_markup=button_main_menu)
    print("Кнопка <Назад> нажата")


@router.callback_query(F.data == 'Назад_2')
async def hello_world(callback : CallbackQuery, state : FSMContext):
    get_cat(f"users_pictures/{callback.from_user.id}/temp.png")
    temp_data = await state.get_data()
...
@router.callback_query(F.data == 'Назад_3')
async def hello_world(callback : CallbackQuery, state : FSMContext):
    media = types.InputMediaPhoto(media=types.FSInputFile(path=f"users_pictures/{callback.from_user.id}/temp.png"), caption="Котик")
    temp_data = await state.get_data()
    await bot.edit_message_media(media=media, chat_id=callback.message.chat.id, message_id=temp_data['cat_message_id'], reply_markup=button_watch_cats)
