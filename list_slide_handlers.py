...
#     await callback.message.answer(text="Пожалуйста напиши номер фотки", reply_markup=button_exit_2)
...
#     temp_data = await state.get_data()
#     path_list = f"users_pictures/{callback.from_user.id}/{temp_data['directory']}"
#     if "!" < 2 or "!" > len(os.listdir(path_list)) or "!" not int:
#         await bot.edit_message_text(text="Пожалуйста введите корректное число", chat_id=callback.message.chat.id, message_id=temp_data['cat_slide_message_id'], reply_markup=button_exit_2)
#     else:
#         media = types.InputMediaPhoto(media=types.FSInputFile(path=f"{path_list}/{sorted(os.listdir(path_list))["!" - 1]}"), caption=f"{"!"} / {len(os.listdir(path_list))}")
#         await bot.edit_message_media(media=media, chat_id=callback.message.chat.id, message_id=temp_data['cat_slide_message_id'], reply_markup=button_watch_list_cats)
#         await state.update_data(picture_number="!")
...
#     temp_data = await state.get_data()
#     await state.clear()
#     await state.update_data(cat_slide_message_id=temp_data['cat_slide_message_id'])
#     await callback.message.delete()
